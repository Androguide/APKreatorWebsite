...
public class BarGraph extends View {
	...
	public void onDraw(Canvas ca) {
		...
		if (fullImage == null || shouldUpdate) {
			...
			float barWidth = (getWidth() - (padding * 2) * points.size())
					/ points.size();
			...
			for (Bar p : points) {
				...
				if (showBarText) {
					...
					Rect r2 = new Rect();
					this.p.getTextBounds(p.getValue() + "m", 0, 1, r2);
					popup.setBounds(
							(int) (((r.left + r.right) / 2) - (this.p
									.measureText(p.getValue() + "m") / 2)) - 14,
							r.top + (r2.top - r2.bottom) - 26,
							(int) (((r.left + r.right) / 2) + (this.p
									.measureText(p.getValue() + "m") / 2)) + 14,
							r.top);
					popup.draw(canvas);
					canvas.drawText(p.getValue() + "m",
							(int) (((r.left + r.right) / 2) - (this.p
									.measureText(p.getValue() + "m") / 2)),
							r.top - 20, this.p);
				}
