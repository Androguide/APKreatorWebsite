...
echo -n -e "${CYAN}Color Scheme (hex)?${RESET}
"
read color
...
Then, simply extract the zip package in this folder${DIR}${RESET}"
	sleep 1
	echo "..."
	sleep 1
	echo "..."
...
echo -e "${GREEN}Building with package name ${package}, app name ${name} and color scheme ${color}...${RESET}"
...
sed -i "s/com.androguide.apkreator/${package}/g" APKreator/build.gradle
sed -i "s/#96AA31/${color}/g" APKreator/res/values/colors.xml
./gradlew build
wait $!
DATE=$(date +"%Y%m%d%H%M");
cp -f APKreator/build/apk/*-release.apk out/$name-release-$DATE.apk
